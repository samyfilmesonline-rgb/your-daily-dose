...
DECLARE
  ...
  v_payload jsonb;
  -- multi-ws
  v_per_ws constant integer := 200; -- mirrors PER_WS in partner-shop-multi-workspace-tick
  v_plan jsonb;
...
BEGIN
  ...
  IF v_order.multi_workspace_mode THEN
    ...
    -- rebuild plan: reset failed/skipped -> pending, keep done
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_plan) LOOP
      IF (v_item->>'status') IN ('failed','skipped','pending','running') AND (v_item->>'status') <> 'done' THEN
  ...
  -- =========================================================
  -- SINGLE-WORKSPACE PATH (original behavior)
  -- =========================================================
