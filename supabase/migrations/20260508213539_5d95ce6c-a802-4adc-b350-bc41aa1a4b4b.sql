...
BEGIN
  ...
  END IF;
  IF NOT v_order.is_manual THEN
    RAISE EXCEPTION 'Pedido não é manual';
  END IF;
  ...
  IF v_order.delivered_at IS NOT NULL THEN
    RAISE EXCEPTION 'Pedido já entregue';
  END IF;

  -- Calcular o que já foi farmado (segurança)
  ...
  IF v_to_redebit <= 0 THEN
    RAISE EXCEPTION 'Nada a re-debitar (já entregue na prática)';
  END IF;
  ...
  -- Re-debitar
  PERFORM public.debit_partner_quota(
    v_order.partner_id,
    v_to_redebit,
    _order_id,
    'manual_retry'
  ...
  -- Atualizar payload com histórico de retries
  ...
  -- Atribuir bot (vai para processing ou queued)
...
REVOKE EXECUTE ON FUNCTION public.retry_manual_order(uuid) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.retry_manual_order(uuid) TO authenticated;